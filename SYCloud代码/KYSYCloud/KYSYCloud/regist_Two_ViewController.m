...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
   ...
   // NSURL* myurl = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@?aguid=%@fileName=%@",HTTPIP,UPLOAD_URL,USER_NAME,myfileName]];
        //[self upLoadSalesBigImage:myfileName MidImage:@"" SmallImage:@""];
    ...
    //NSString *myfileName=[NSString stringWithFormat:@"%lu.jpg" ,(unsigned long)_picArray.count];
    
    //[self saveImage:newImage WithName:myfileName];
...
-(void)keepRequestwithGuid:(NSString*)guid withcmdType:(NSString*)cmdtype{
    ...
    NSLog(@"myguif=====%@",myguid);
    if ([guid isEqualToString:@""]||[CUSTOMERINFO isEqualToString:@""]) {
        [SVProgressHUD showErrorWithStatus:@"未找到该扫描码所关联的设备信息。"];
        return;
    }
    ...
    if ([manager fileExistsAtPath:AudioPath]) {
         ...
         audioStr =[NSString stringWithFormat:@"<FILE><FILENAME>%@</FILENAME><FILETYPE>1</FILETYPE><FILEPATH>%@%@.mp3</FILEPATH><FILEEXISTENION>.mp3</FILEEXISTENION></FILE>",myfileName,SRVER_FILE_LOGINCARTION,myfileName];
